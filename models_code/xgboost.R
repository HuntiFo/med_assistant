...
# Convert character columns to factors, target to factor
...
# Create model matrices for XGBoost, exclude intercept and column "X" if exists
train_matrix <- model.matrix(target ~ . - X, data = train_data)[, -1]
test_matrix <- model.matrix(target ~ . - X, data = test_data)[, -1]
...
#saveRDS(xgb_model, "xgb_model_no_smote.rds")
